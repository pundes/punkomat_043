...
class Picture {
  ...
  void cropTiles(int count) {
    ...
    //3.Reihe
    if(count == 18) {
       randX = map(noise(time), 0, 1, location.x - 25, location.x  + 10); 
       randY = map(noise(time), 0, 1, location.y, location.y + 4);
       cropPics();
    } 
    ...
    //4.Reihe
    if(count == 24) {
       randX = map(noise(time), 0, 1, location.x, location.x  + 1); 
       randY = map(noise(time), 0, 1, location.y -8, location.y + 12);
       cropPics();
    } 
    ...
    if(count == 26) {
       ...
       randY = map(noise(time), 0, 1, location.y -2, location.y);
       cropPics();
    } 
    
    if(count == 27) {
       randX = map(noise(time), 0, 1, location.x - 8, location.x  + 2); 
       randY = map(noise(time), 0, 1, location.y -1, location.y);
       cropPics();
    } 
    
    if(count == 28) {
       randX = map(noise(time), 0, 1, location.x, location.x  + 3); 
       randY = map(noise(time), 0, 1, location.y -10, location.y + 8);
       cropPics();
    }
    
    if(count == 29) {
       randX = map(noise(time), 0, 1, location.x - 7, location.x  + 7); 
       randY = map(noise(time), 0, 1, location.y, location.y + 3);
       cropPics();
    } 
    
    //5.Reihe
    if(count == 33) {
       randX = map(noise(time), 0, 1, location.x - 10, location.x  + 12); 
       randY = map(noise(time), 0, 1, location.y, location.y + 4);
       cropPics();
    } 
    ...
    if(count == 35) {
       randX = map(noise(time), 0, 1, location.x -15, location.x  + 15); 
       randY = map(noise(time), 0, 1, location.y -4, location.y + 4);
       cropPics();
    }
    ...
    if(count == 36) {
       ...
       randY = map(noise(time), 0, 1, location.y -8, location.y + 10);
       cropPics();
    } 
    ...
    if(count == 45) {
       randX = map(noise(time), 0, 1, location.x - 2, location.x); 
       randY = map(noise(time), 0, 1, location.y - 2, location.y + 3);
       cropPics();
    }
    ...
    if(count == 52) {
       randX = map(noise(time), 0, 1, location.x, location.x  + 1); 
       randY = map(noise(time), 0, 1, location.y -5, location.y + 2);
       cropPics();
    }
    time = time + 0.03;
